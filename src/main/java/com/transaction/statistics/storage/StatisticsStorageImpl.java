...
public class StatisticsStorageImpl<T> implements StatisticsStorage<T> {
    ...
    public static <T> StatisticsStorage<T> lastMinute(Supplier<T> factory) {
        return new StatisticsStorageImpl<>(ChronoUnit.MINUTES, ChronoUnit.SECONDS, 64, factory);
    }

    private StatisticsStorageImpl(TemporalUnit targetUnit, TemporalUnit groupingUnit, int bufferSize, Supplier<T> factory) {
        this(targetUnit, groupingUnit, bufferSize, factory, System::currentTimeMillis);
    }

    StatisticsStorageImpl(TemporalUnit targetUnit, TemporalUnit groupingUnit, int bufferSize, Supplier<T> factory, Supplier<Long> now) {
        this.now = now;
